
<!DOCTYPE html>
<html lang="en" dir="ltr">
  <head>
    <meta charset="utf-8">
    <title>{{$nota->nama}} - {{$nota->kode}}</title>
    <link href="{{ public_path("/css/bootstrap.css")}}" rel="stylesheet">
  </head>
  <body>
    <div class="card">
      <div class="card-body shadow">
        <table class="table table-borderless">
          <thead>
            <tr>
              <th scope="col"><img src="data:image/png;base64," class="img-fluid">
              </th>
              <th class="text-right align-middle" scope="col" style="margin: 0px auto;">
                <address class="font-weight-normal">
                  <strong>Invoice #: 000{{$nota->id}}</strong><br>
                  Created: {{$nota->created_at->toFormattedDateString()}}<br>
                  Due: {{$nota->tanggal_kembali->toFormattedDateString()}}<br>
                  Code: <strong class="text-monospace">{{$nota->kode}}</strong>
                </address>
              </th>
            </tr>
          </thead>
        </table>
        <table class="table table-borderless">
          <thead>
            <tr>
              <th scope="col">
                <address class="font-weight-normal">
                  5758 Media<br>
                  Perum. Semeru Blok H no.6<br>
                  Sumbersari, Jember, 68121<br>
                  <abbr title="Phone">P:</abbr> (62) 89516156911
                </address>
              </th>
              <th class="text-right align-middle" scope="col" style="margin: 0px auto;">
                <address class="font-weight-normal">
                  {{$nota->nama}}<br>
                  {{$nota->acara}}, {{$nota->alamat}}<br>
                  <abbr title="Phone">P:</abbr> (62) {{$nota->no_hp}}
                </address>
              </th>
            </tr>
          </thead>
        </table>

        <table class="table table-sm">
          <thead class="thead-light">
            <tr>
              <th scope="col">
                <strong>Order Detail</strong><br>
              </th>
              <th class="text-right align-middle" scope="col">
              <strong>Check#</strong><br></th>
            </tr>
          </thead>
          <tbody>
            <tr>
              <th scope="row"><span class="font-weight-normal text-muted">Package</span>
                @if ($nota->paket == 1)
                    <span class="font-weight-bold">Normal</span>
                @elseif ($nota->paket == 2)
                    <span class="font-weight-bold">Media Partner</span>
                @else
                    <span class="text-muted">ERROR</span>
                @endif
              </th>
              <td class="text-right align-middle">
                    <p class="font-weight-bold">Rp. {{$nota->harga_unit}}/Unit/24Jam</p>
              </td>
            </tr>
            <tr>
              <th scope="row"><span class="font-weight-bold">Duration, <span class="font-weight-normal">for</span> {{$nota->tanggal_sewa->toDateString()}} <span class="font-weight-normal">until</span> {{$nota->tanggal_kembali->toDateString()}} </span>
              </th>
              <td class="text-right align-middle">
                    <p class="font-weight-bold">{{$nota->jumlah_hari}} Days</p>
              </td>
            </tr>
            <tr>
              <th scope="row"><span class="font-weight-bold">Unit</span>
              </th>
              <td class="text-right align-middle">
                    <p class="font-weight-bold">{{$nota->jumlah_unit}} Units</p>
              </td>
            </tr>
            <tr>
              <th scope="row"><span class="font-weight-bold">Total order</span>
              </th>
              <td class="text-right align-middle">
                @if (isset($nota->total_pembayaran))
                    <p class="font-weight-bold">Rp. {{$nota->total_pembayaran}}</p>
                @elseif (is_null($nota->total_pembayaran))
                    on process
                @else
                    <p class="font-weight-bold">ERROR</p>
                @endif
              </td>
            </tr>

          </tbody>
        </table>
        <table class="table table-sm">
          <thead class="thead-light">
            <tr>
              <th scope="col">
                <strong>Payment Method</strong><br>
              </th>
              <th class="text-right align-middle" scope="col">
              <strong>Check#</strong><br></th>
            </tr>
          </thead>
          <tbody>
            <tr>
              <th scope="row">Down payment
                @if (isset($nota->tanggal_dp))
                    <span class="font-weight-normal text-muted">,{{$nota->tanggal_dp->toFormattedDateString()}}</span>
                @elseif (is_null($nota->tanggal_dp))
                    on process
                @else
                    <span class="text-muted">ERROR</span>
                @endif
              </th>
              <td class="text-right align-middle">
                @if (isset($nota->dp))
                    <p class="font-weight-bold">Rp. {{$nota->dp}}</p>
                @elseif (is_null($nota->pelunasan))
                    0
                @else
                    <p class="font-weight-bold">ERROR</p>
                @endif
              </td>
            </tr>
            <tr>
              <th scope="row">Full payment
                @if (isset($nota->tanggal_pelunasan))
                    <span class="font-weight-normal text-muted">,{{$nota->tanggal_pelunasan->toFormattedDateString()}}</span>
                @elseif (is_null($nota->tanggal_pelunasan))
                    on process
                @else
                    <span class="text-muted">ERROR</span>
                @endif
              </th>
              <td class="text-right align-middle">
                @if (isset($nota->pelunasan))
                    <p class="font-weight-bold">Rp. {{$nota->pelunasan}}</p>
                @elseif (is_null($nota->pelunasan))
                    0
                @else
                    <p class="font-weight-bold">ERROR</p>
                @endif
              </td>
            </tr>
            <tr>
              <th scope="row"><span class="font-weight-bold">Total payment</span>
              </th>
              <td class="text-right align-middle">
                @if (isset($nota->total_pembayaran))
                    <p class="font-weight-bold">Rp. {{($nota->dp + $nota->pelunasan)}}</p>
                @elseif (is_null($nota->total_pembayaran))
                    0
                @else
                    <p class="font-weight-bold">ERROR</p>
                @endif
              </td>
            </tr>
          </tbody>
        </table>

        <br> <br> <table class="table table-borderless table-sm">
          <thead>
            <tr>
              <th scope="col">
                <strong>STATUS:</strong><br>
              </th>
              <th class="text-right" scope="col">
              <strong>CODE:</strong><br></th>
            </tr>
          </thead>
          <tbody>
            <tr>
              <th scope="row">
                @if($nota->status =='0')
                        <h3>Menunggu di proses</h3>
                  @elseif($nota->status =='1')
                        <h3>Proses pembayaran</h3>
                  @elseif($nota->status =='2')
                        <h3>Pembayaran di konfirmasi penyewa</h3>
                  @elseif($nota->status =='3')
                        <h3>Proses jaminan</h3>
                  @elseif($nota->status =='4')
                        <h3>Jaminan di konfirmasi penyewa</h3>
                  @elseif($nota->status =='5')
                        <h3>Sewa berjalan</h3>
                  @elseif($nota->status =='6')
                        <h3>Sewa selesai</h3>
                  @elseif($nota->status =='98')
                        <h3>Proses pembatalan order</h3>
                  @elseif($nota->status =='99')
                        <h3>Pesanan dibatalkan</h3>
                  @else
                        <h3>ERROR</h3>
                @endif
              </th>
              <td class="text-right align-middle">
                <h3 class="text-monospace">{{$nota->kode}}</h3>
              </td>
            </tr>
          </tbody>
        </table>
        <table class="table table-borderless table-sm">
          <thead>
            <tr>
              <th scope="col">
                <strong>JAMINAN:</strong><br>
              </th>
            </tr>
          </thead>
          <tbody>
            <tr>
              <th scope="row">
                @if (isset($nota->jenis_jaminan))
                    <p class="font-weight-bold">{{$nota->jenis_jaminan}}</p>
                @elseif (is_null($nota->jenis_jaminan))
                    On process
                @else
                    <p class="font-weight-bold">ERROR</p>
                @endif
              </th>
            </tr>
          </tbody>
        </table>

        <br> <br> <p class="text-muted">*Lampiran ini dikeluarkan dan bersifat resmi oleh pihak 5758Media sebagai tanda bukti pembayaran dan pengganti nota pembayaran yang telah ada sebelumnya.</p>

      </div>
    </div>
  </body>
</html>
